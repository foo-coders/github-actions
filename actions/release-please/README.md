...
7. Create the environment that will hold them: repo → **Settings** → **Environments** → **New environment** (e.g. `release`). Under **Deployment branches and tags**, choose **Selected branches and tags** and add a rule matching your release branch (e.g. `main`). Skipping this step leaves the environment restricting nothing.
8. In that environment, **Add environment secret** twice, named `RELEASE_PLEASE_CLIENT_ID` and `RELEASE_PLEASE_PRIVATE_KEY`, matching the usage example above. Repository or organization secrets work too, but leave the private key readable from every branch — see [Protecting the credentials](#protecting-the-credentials).
...
An environment closes that. Its **deployment branch policy** limits which refs may access its secrets, so restricting the environment to your release branch means the key is only reachable from code that has already passed review on that branch. Create it as described in [Generating the GitHub App credentials](#generating-the-github-app-credentials) above, then name it on the job:
...
If you point this action at a maintenance branch with `target-branch`, that branch must be allowed by the deployment branch policy too. Otherwise the secrets arrive empty and the token step fails with an unhelpful error rather than a permission one.
