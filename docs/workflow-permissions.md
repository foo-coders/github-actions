...
Document each granted scope with a comment on the same line, naming what reads it (`pull-requests: read # the title check reads the pull request`). `contents: read` is exempt — the checkout above it says the same thing. Only the same-line form documents a scope: neither this convention nor `zizmor`'s `undocumented-permissions` accepts a comment on the line above. That audit is pedantic-only, so `mise run lint` enforces none of this. A comment above the block does a different job — explaining a scope the job deliberately does **not** grant, as in `release.yml`.
